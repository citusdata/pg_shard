...
-- create a custom type...
CREATE TYPE dummy_type_2 AS (
    i integer
);

-- ... as well as a function to use as its comparator...
CREATE FUNCTION dummy_type_2_function(dummy_type_2, dummy_type_2) RETURNS boolean
AS 'SELECT TRUE;'
LANGUAGE SQL
IMMUTABLE
RETURNS NULL ON NULL INPUT;
...
-- ... use that function to create a custom operator...
CREATE OPERATOR = (
    LEFTARG = dummy_type_2,
    RIGHTARG = dummy_type_2,
    PROCEDURE = dummy_type_2_function
);

-- ... and create a custom operator family for hash indexes...
CREATE OPERATOR FAMILY dummy_2_op_family USING hash;

-- create a hash function for the custom composite type
CREATE FUNCTION DUMMY_HASH(dummy_type_2)
RETURNS int LANGUAGE internal AS 'hashtext' IMMUTABLE;

-- ... finally, build an operator class, designate it as the default operator
-- class for the type with an equality operator and hash function. So the type will
-- have a default op class WITH hash operator in that class.
CREATE OPERATOR CLASS dummy_2_op_family_class
DEFAULT FOR TYPE dummy_type_2 USING hash FAMILY dummy_2_op_family AS
OPERATOR 1 =,
FUNCTION 1 DUMMY_HASH(dummy_type_2);

-- test for composite type partioning 
...
SELECT master_create_distributed_table('composite_type_partitioned_table', 'c1');

SELECT master_create_worker_shards('composite_type_partitioned_table', 16, 1);
